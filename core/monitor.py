...
def extract_daily_metrics(snapshot: dict) -> Optional[dict]:
    ...
    spo2_val = None
    if spo2 is not None:
        raw = getattr(spo2, "spo2", None) or str(spo2)
        import re
...
class HealthMonitor:
    ...
    async def _tick(self):
        ...
        for sid in sessions:
            ...
            # 只发第一个未冷却的关怀(避免刷屏)
            alert = alerts[0]
            text = await self._compose_message(alert, session_id=sid)
            if not text:
                continue
            ok = await self._send(sid, text)
            if ok:
    ...
    # ---------- 文案生成 ----------
    async def _compose_message(self, alert: dict, session_id: str | None = None) -> str:
        """尽量走 LLM, 失败兜底成模板。session_id 用于加载最近对话/人设。"""
        hint = alert.get("hint", "")
        key = alert.get("key", "")
        try:
            provider = self.plugin.context.get_using_provider()
            if provider is None:
                return self._fallback_text(alert)

            # 允许用户在配置里自定义 prompt 模板, 支持占位符 {hint} {key} {time} {date}
            template = self.config.get("care_prompt_template") or (
                "[内心独白] 我刚扫了眼她今天的身体数据, 注意到: {hint}。\n"
                "现在时间 {time}, 我想以我自己的口吻自然地对她说一句关心。\n\n"
                "严格要求:\n"
                "- 保持我一贯的说话方式, 不要用夸张的语气词、不要连续叠字撒娇\n"
            ...
            try:
                ...
                prompt = f"[后台提示] 我注意到: {hint}。请以我自己的口吻自然地关心她一句。"

            # 1) 系统提示词: 优先自定义, 否则按开关继承默认人设
            system_prompt = await self._resolve_care_system_prompt(session_id, provider)

            # 2) 最近对话: 按开关携带
            contexts = await self._resolve_care_contexts(session_id)
            ...
            return text or self._fallback_text(alert)
        except Exception as e:
            logger.warning(f"[mi_health.monitor] LLM 生成关怀文案失败, 走兜底: {e}")
            return self._fallback_text(alert)

    async def _resolve_care_system_prompt(self, session_id: str | None, provider) -> str:
        """决定主动关怀调用 LLM 时用哪段 system_prompt。"""
        # 1. 用户手动覆盖优先
        custom_sys = (self.config.get("care_system_prompt") or "").strip()
        if custom_sys:
            return custom_sys

        # 2. 开关关掉 -> 不带人设
        if not bool(self.config.get("care_use_persona", True)):
            return ""

        # 3. 尝试从当前会话对应的 persona 拿到人设
        try:
            if session_id:
                cm = self.plugin.context.conversation_manager
                cid = await cm.get_curr_conversation_id(session_id)
                if cid:
                    conv = await cm.get_conversation(session_id, cid)
                    persona_id = getattr(conv, "persona_id", None) if conv else None
                    if persona_id and persona_id != "[%None]":
                        pm = getattr(self.plugin.context, "persona_manager", None)
                        if pm:
                            try:
                                persona = await pm.get_persona(persona_id)
                            except Exception:
                                persona = None
                            if persona is None and hasattr(pm, "personas"):
                                for p in pm.personas or []:
                                    if getattr(p, "name", None) == persona_id or getattr(p, "id", None) == persona_id:
                                        persona = p
                                        break
            ...
            logger.debug(f"[mi_health.monitor] 读取 persona 失败: {e}")

        # 4. 兜底: 尝试直接从 provider 拿默认 system_prompt
        try:
            for attr in ("curr_personality", "personality", "system_prompt"):
                val = getattr(provider, attr, None)
                if isinstance(val, dict):
                    val = val.get("prompt") or val.get("content")
    ...
    async def _resolve_care_contexts(self, session_id: str | None) -> list:
        """按配置携带最近 N 轮对话上下文。"""
        if not bool(self.config.get("care_use_recent_context", True)):
            return []
        if not session_id:
            return []
        try:
            turns = max(1, int(self.config.get("care_context_turns", 4)))
        except Exception:
            turns = 4
        try:
            cm = self.plugin.context.conversation_manager
            ...
            # 取末尾 2 * turns 条 (一来一回算一轮)
            return clean[-(turns * 2):]
        except Exception as e:
            logger.debug(f"[mi_health.monitor] 读取最近对话失败: {e}")
            return []
    ...
    async def _send(self, session_id: str, text: str) -> bool:
        try:
            ...
            logger.exception(f"[mi_health.monitor] 发送失败 session={session_id}: {e}")
            return False
    ...
    async def _tick_force(self, session_id: str | None = None) -> None:
        ...
        # 5. 直接发第一个 alert (跳过冷却)
        alert = alerts[0]
        _first_sid = sessions[0] if sessions else None
        text = await self._compose_message(alert, session_id=_first_sid)
        if not text:
            raise RuntimeError("compose empty")
